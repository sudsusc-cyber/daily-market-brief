...
# 兼容 LLM 输出的多种引用变体:
#   <sup>[N]</sup> / <sup>(N)</sup> / <sup>【N】</sup> / <sup>(N)</sup>  ← 标准上标
#   [N] / 【N】 — 后不跟字母数字时认作引用                              ← 裸方括号 + 中文括号
#   ^N^                                                                  ← markdown
# 注意:**裸半角 `(N)` 与裸全角 `(N)` 不当脚注**——中文/英文文本里
# "iOS (14)"、"步骤 (1) 完成"、"(图 3) 展示" 等场景太常见,误匹配会把它们
# 当脚注吞掉。需要圆括号脚注时 LLM 必须用 <sup>(N)</sup> 形式包起来,语义明确。
