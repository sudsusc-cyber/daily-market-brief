...
**当前阶段**:M4(LLM 处理层)— ✅ 已通过(用户在 2026-04-30 切 session 准备 M5,M4 视为已通过)
...
| M3 | 数据采集层完善 | ✅ 已通过 | 2026-04-30 | 2026-04-30 | 5 模块原始数据 + 标题翻译 + 相关性过滤,30 tests pass,详见 ADR-0004 / ADR-0005 |
| M4 | LLM 处理层 | ✅ 已通过 | 2026-04-30 | 2026-04-30 | 4 个 processor + 多轮 typography/prompt 内修复,47 tests pass,详见 ADR-0006 / ADR-0008 |
| M5 | HTML 跨客户端兼容性 | ⬜ 待开始(新 session) | — | — | 精美设计已 M2 完成,M5 范围收窄到兼容性 + 微调,见 ADR-0008 §C |
| M6 | 部署上线 + 稳定性 | ⬜ 待开始 | — | — | GH Actions cron 已锁定北京周二-周六 08:00,详 ADR-0007 |
...
| 用户人工审阅"原始数据邮件"确认数据真实可信 | ✅ "很棒了现在" | 经多轮迭代:TSM 方形 + Reuters→CNBC + 翻译 + 相关性过滤 |

### 验收期间用户提出的补丁(详见 ADR-0005)

1. **TSM 在手机端 logo 不显示** → 改用 FMP 250×250 方形(原 Wikimedia 960×757 非方形)
2. **Reuters RSS 已停服** → 替换为 CNBC Top News(财经向)
3. **国外标题英文,希望中文** → 提前接 DeepSeek 批量翻译(范围调整,M4 时整合进 processors/)
4. **Finnhub 同板块新闻噪音重(NVDA 249 条只有 1 条真相关)** → 加 `_RELEVANCE_KEYWORDS` 过滤,total 567→199 条
